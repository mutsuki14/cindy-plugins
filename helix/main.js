...
// Built from source/gate-core.js + source/adapter-cindy.js by scripts/build-gates.mjs.
// Edit those sources and re-run the build; hand-edits here will be overwritten.

// Helix gate core — SINGLE SOURCE OF TRUTH for every deterministic gate ruling.
//
// Pure: no IO, no globals, no host APIs. Both shipped calculators
// (Cindy plugin main.js and the standalone scripts/gate.mjs) are GENERATED
// from this file by scripts/build-gates.mjs. Never hand-edit the generated
// copies — the drift that hand-syncing caused is exactly why this file exists.
//
// decide(args, opts) -> { ok: true, result } | { ok: false, error }
//   opts.thresholdHint  host-specific wording in the calibrate recommendation
//   opts.extraModes     modes the adapter implements on top of the core
//                       (currently only 'evidence', standalone-only)
...
const CORE_MODES = ['ambiguity', 'drift', 'loop', 'receipt', 'calibrate', 'triage'];

function round(n) {
  return Math.round(n * 1000) / 1000;
}

function decide(args, opts) {
  const o = opts || {};
  const thresholdHint = o.thresholdHint === undefined ? '传 ' : o.thresholdHint;
  const allModes = CORE_MODES.concat(o.extraModes || []);
  const bad = (error) => ({ ok: false, error });
  const good = (result) => ({ ok: true, result });
  ...
  if (args.mode === 'ambiguity') {
    ...
    if (threshold === null) return bad(`ambiguity 门槛只能收紧：threshold 须在 0.05–${GATES.ambiguity.threshold} 之间`);
    const weights = args.brownfield ? GATES.ambiguity.brownfield : GATES.ambiguity.greenfield;
    const contributions = {};
    let claritySum = 0;
    for (const name of Object.keys(weights)) {
      const v = dim(name);
      if (v === null) return bad(`ambiguity 模式缺少或非法维度分 "${name}"(0–1)${name === 'context' ? '；棕地模式必须提供 context' : ''}`);
  ...
  if (args.mode === 'drift') {
    ...
    if (threshold === null) return bad(`drift 门槛只能收紧：threshold 须在 0.1–${GATES.drift.threshold} 之间`);
    const weights = GATES.drift.weights;
    const contributions = {};
    let score = 0;
    for (const name of Object.keys(weights)) {
      const v = dim(name);
      if (v === null) return bad(`drift 模式缺少或非法维度分 "${name}"(0–1)`);
  ...
  if (args.mode === 'loop') {
    ...
    if (!Number.isInteger(total) || total < 1) return bad('loop 模式需要 ac_total(≥1 的整数)');
    if (!Array.isArray(history) || history.length < 1 || history.some((n) => !Number.isInteger(n) || n < 0 || n > total)) {
      return bad('loop 模式需要 ac_passed_history(非空整数数组,每项 0–ac_total)');
    }
  ...
  if (args.mode === 'receipt') {
    ...
    if (!Array.isArray(checks) || checks.length < 1 || checks.some((c) =>
      ...
      return bad('receipt 模式需要 checks(非空数组,每项 {name, exit_code, fresh, scope: target|regression|other})');
    }
    ...
    if (args.uncovered !== undefined &&
      ...
      return bad('receipt 模式的 uncovered 必须是字符串数组（只有一条也要写成 ["..."]）——传字符串或混入非字符串会被当作"无未覆盖面"错误抬高评级');
    }
    // 带 incomplete 标记的检查来自被杀进程，exit_code 是 fail-closed 占位值而非真实退出码。
    const incompleteChecks = checks.filter((c) => typeof c.incomplete === 'string' && c.incomplete).map((c) => c.name);
    const uncovered = args.uncovered === undefined ? [] : args.uncovered;
    ...
    if (incompleteChecks.length) {
      grade = 'C';
      reason = `有检查未跑完（${incompleteChecks.join(', ')}）：退出码不可信，先重跑再定级`;
    ...
    return good({
      ...
      stale,
      uncovered,
      ...(incompleteChecks.length ? { incomplete: incompleteChecks } : {}),
      verdict: grade === 'C'
  ...
  if (args.mode === 'triage') {
    ...
    for (const b of bools) {
      if (typeof args[b] !== 'boolean') return bad(`triage 模式七个信号全部必填（强迫逐项过脑）：缺 "${b}"(boolean)`);
    }
    const amb = args.ambiguity_guess;
    if (typeof amb !== 'number' || amb < 0 || amb > 1) return bad('triage 模式缺 "ambiguity_guess"(0–1 数字，粗估即可)');
  ...
  if (args.mode === 'calibrate') {
    ...
    if (!Array.isArray(records) || records.some((r) =>
      ...
      return bad('calibrate 模式需要 records(数组,每项 {ambiguity: 0–1, loops: ≥1 整数, drift?: 0–1})');
    }
    ...
    if (stats.triage && stats.triage.with_levels >= 5 && stats.triage.under_triaged / stats.triage.with_levels > 0.3) {
      ...
      recommendation = `自评偏乐观：高返工任务的歧义分平均 ${stats.high_rework.mean_ambiguity}，当时都过了门却平均返工 3+ 圈。建议此类任务门槛收紧为 ${suggested}（下次 clarify 时${thresholdHint}threshold: ${suggested}），并把本建议记入 learnings。`;
    } else {
      recommendation = '高返工任务的歧义分本就超标（>0.2）——问题不在门槛在执行：检查是否存在未过门就动工的情况。';
    }
    return good({ gate: 'calibrate', ...stats, recommendation });
  }

  return bad(`mode 必须是 ${allModes.map((m) => `"${m}"`).join('、')}`);
}
